...
VisuMZ.TileD.version = 1.04;

//=============================================================================
/*:
* @target MZ
* @plugindesc [RPG Maker MZ] [Tier 5] [Version 1.04] [TileD]
* @author VisuStella
...
*
* ◈ Version 1.04:
*  Fixed a bug where it crashes when opening the menu.
*
* ◈ Version 1.03:
*  zIndex' property now works on folders.
*
* ◈ New Features:
* A new property named 'hiddenInGame' is added. This hides the layer from the map. 
* Useful for certain features like flags.
* 
* Fixed a bug where hitting an edge without a collision tile will cause a crash.
* Fixed a bug with the property 'terrainTag' not working.
*
* 
* Version 1.02:
...
VisuMZ.Utility.getProperty = function (properties, propertyName) {
    ...
    let property = properties.find(function (prop) {
        return prop.name.toLowerCase() === propertyName.toLowerCase();
    });
    if (property) {
...
Sprite_TiledDoodad.prototype.setInitialAnimation = function() {
    if (!this._data || (this._data && !this._data._animData)) return;
...
TiledTilemap.prototype.isPaintLayer = function (layerData) {
    ...
    if (!!properties) {
        if (!!VisuMZ.Utility.getProperty(properties, 'collision') ||
            ...
            !!VisuMZ.Utility.getProperty(properties, "hiddenInGame"))
            return false;
    }
    return true;
};
...
DataManager.recursiveExtractLayers = function (groupLayer, extracted) {
    ...
    const hiddenInGame = (VisuMZ.Utility.getProperty(groupLayer.properties, "hiddenInGame")) || false;
...
Game_Map.prototype._getLayerProperties = function (layerData) {
    ...
    const hiddenInGame = VisuMZ.Utility.getProperty(properties, "hiddenInGame");
    if (hiddenInGame != null) layerData.hiddenInGame = hiddenInGame || false;
...
Game_Map.prototype._setupTileFlags = function (layerData, index) {
    ...
    if (tileProperties.terrainTag) {

        flags[index].push(Number(tileProperties.terrainTag) << 12);
    }
};
...
Game_Map.prototype.checkPassageColliders = function (x, y, bit) {
    ...
    if (colliderMap) {
        ...
        return colliderMap[index] ? colliderMap[index].full === 0 : true;
    }
    return true;
};
